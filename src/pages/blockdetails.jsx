                ...
                <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                    <dl className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <dd className="mt-1 text-gray-800 dark:text-neutral-200 lowercase break-all">{block.header.hash}</dd>
                        </div>
                        <div>
                        ...
                        {/* Add more fields if available, e.g., timestamp */}
                    ...
                    {block.transactions && block.transactions.length > 0 ? (
                        <ul className="space-y-3">
                            {block.transactions.map((tx, index) => (
                                <li key={tx.hash || `reward-${index}`} className="bg-gray-50 p-3 rounded-lg dark:bg-gray-700">
                                    <div className="flex justify-between items-center">
                                        <span className="text-sm font-medium text-gray-800 dark:text-neutral-200 break-all">
                                            {tx.hash ? abbreviate(tx.hash) : 'Coinbase Reward'}
                                        </span>
                                        {/* Add more tx details if available */}
                                        {tx.hash && (
                                            <a
                                                href={`/chain/transaction/${tx.hash}`}
                                    ...
                                    {/* Optionally add more details like from/to/amount */}
                                    {tx.fromAddress && (
